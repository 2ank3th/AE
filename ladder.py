...
inputs = torch.FloatTensor(0, layer_sizes[0])
outputs = torch.FloatTensor()
#inputs = tf.placeholder(tf.float32, shape=(None, layer_sizes[0]))
#outputs = tf.placeholder(tf.float32)

def bi(inits, size, name):
    return Variable(inits * torch.ones([size]), name=name)
    #return tf.Variable(inits * tf.ones([size]), name=name)


def wi(shape, name):
    return Variable(torch.randn(shape, name=name)) / math.sqrt(shape[0])
...
running_mean = [torch.Variable(torch.zeros(shape=[l]), trainable=False) for l in layer_sizes[1:]]
running_var = [torch.Variable(torch.ones(shape=[l]), trainable=False) for l in layer_sizes[1:]]
...
"""
"""
Encode and Decode from another model.
def encode(self, x):
...
def encoder(inputs, noise_std):
    h = inputs + torch.randn(torch.size(inputs)) * noise_std  # add noise to input
    ...
    for l in range(1, L+1):
        ...
        z_pre = torch.matmul(h, weights['W'][l-1])  # pre-activation
        ...
        def training_batch_norm():
            ...
            if noise_std > 0:
                ...
                z += torch.randn(torch.size(z_pre)) * noise_std
        ...
        if l == L:
            ...
            h = torch.nn.softmax(weights['gamma'][l-1] * (z + weights["beta"][l-1]))
            #h = tf.nn.softmax(weights['gamma'][l-1] * (z + weights["beta"][l-1]))
        else:
            # use ReLU activation in hidden layers
            h = torch.nn.relu(z + weights["beta"][l-1])
...
for l in range(L, -1, -1):
    ...
    d_cost.append((torch.mean(torch.sum((z_est_bn - z) * (z_est_bn - z), 1)) / layer_sizes[l]) * denoising_cost[l])
...
pred_cost = -torch.mean(torch.sum(outputs*torch.log(y), 1))  # cost used for prediction
#pred_cost = -tf.reduce_mean(tf.reduce_sum(outputs*tf.log(y), 1))  # cost used for prediction

correct_prediction = torch.eq(torch.argmax(y, 1), torch.ArgMax(outputs, 1))  # no of correct predictions
accuracy = torch.mean(correct_prediction:float()) * torch.Variable(100.0)
#correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(outputs, 1))  # no of correct predictions
#accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float")) * tf.constant(100.0)

learning_rate = torch.Variable(starter_learning_rate, trainable=False)
